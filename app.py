...
def ensure_jwt_token_sync(region):
    ...
    with jwt_lock:
        ...
        # Map regions to UIDs and passwords for the new JWT API
        region_config = {
            "IND": {"uid": "3828066210", "password": "C41B0098956AE7B79F752FCA873C747060C71D3C17FBE4794F5EB9BD71D4DA95"},
            "BR": {"uid": "4345418798", "password": "JOBAYAR_GK6VJ"},
            "US": {"uid": "3787481313", "password": "JlOivPeosauV0l9SG6gwK39lH3x2kJkO"},
            "SAC": {"uid": "4349229968", "password": "GARENA_KI_MKC_50WO1_BY_KALLU_CODEX_22WFM"},
            "BD": {"uid": "4349237175", "password": "GARENA_KI_MKC_TH38G_BY_KALLU_CODEX_HYF2H"},
            "ID": {"uid": "4349239376", "password": "GARENA_KI_MKC_2RTZ5_BY_KALLU_CODEX_GTYZX"},
            "PK": {"uid": "4349240944", "password": "GARENA_KI_MKC_1VK2D_BY_KALLU_CODEX_53S3N"},
            "VN": {"uid": "4349242942", "password": "GARENA_KI_MKC_B9L28_BY_KALLU_CODEX_HQ3T8"},
            "ME": {"uid": "4349244853", "password": "GARENA_KI_MKC_MFD4N_BY_KALLU_CODEX_2Y9F4"},
            "TH": {"uid": "4349247913", "password": "GARENA_KI_MKC_2123L_BY_KALLU_CODEX_SCKTB"},
            "default": {"uid": "4349249859", "password": "GARENA_KI_MKC_VO3QR_BY_KALLU_CODEX_RTAWR"}
        }

        config = region_config.get(region, region_config["default"])
        
        # New JWT API endpoint
        url = f"https://aman-jwt-live.lovable.app/api/public/token?uid={config['uid']}&password={config['password']}"
        ...
        try:
            ...
            # Get TTL from response or use default 28800 seconds (8 hours)
            ttl = data.get("ttl", 28800)
            
            # Also extract other useful info if needed
            release_version = data.get("releaseVersion", "OB54")
            server_url = data.get("serverUrl", "")
            
            print(f"[JWT] Server URL: {server_url}")
            print(f"[JWT] Release Version: {release_version}")

            if token:
                jwt_tokens[region] = token
                # Set expiry based on TTL, with 5 minute buffer
                jwt_expiry[region] = current_time + (ttl - 300) if ttl > 300 else current_time + 300
                print(f"[JWT] Token for {region} updated successfully. Expires in {ttl} seconds.")
                return token
            else:
                print(f"[JWT] Failed to extract token for {region}")
                print(f"[JWT] Response data: {data}")

        except requests.exceptions.RequestException as e:
            print(f"[JWT] Request error for {region}: {e}")
        except ValueError as e:
            print(f"[JWT] JSON parsing error for {region}: {e}")
